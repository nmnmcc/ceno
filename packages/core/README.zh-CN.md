...
低层 `Document` 服务接受和返回 `unknown`。如需类型安全的文档访问，ceno 提供了 `SchemaDocument` 和 `SchemaLocalDocument`：Schema 感知的封装，**写入时编码**，**读取时解码（并自动迁移）**。

### SchemaDocument

以 Effect Schema 字段定义文档结构，然后用 `SchemaDocument.make` 创建类型化的访问器：
